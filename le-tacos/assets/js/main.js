(function () {
  ...
  if (preloader) {
    var introSeen = false;
    try { introSeen = sessionStorage.getItem("leTacosIntroSeen") === "1"; } catch (err) { introSeen = false; }
    if (introSeen || prefersReducedMotion) {
      preloader.remove();
    } else {
      try { sessionStorage.setItem("leTacosIntroSeen", "1"); } catch (err) { /* noop */ }
  ...
  if (cursorDot && canHover) {
    ...
    deriveCursorLabels();
    window.LeTacosRefreshCursor = deriveCursorLabels;
  ...
  if (reservationForm) {
    reservationForm.addEventListener("submit", function (e) {
      e.preventDefault();
      var date = reservationForm.elements.date.value;
      var heure = reservationForm.elements.heure.value;
      var personnes = reservationForm.elements.personnes.value;
      ...
      var lines = [
        "Bonjour ! Je voudrais passer une commande chez Le Tacos.",
        "Date: " + (dateFormatted || "-"),
        "Heure: " + (heure || "-"),
        "Nombre de menus: " + (personnes || "-")
      ];
      if (note) lines.push("Note: " + note);

      var message = encodeURIComponent(lines.join("\n"));
      window.open("https://wa.me/33437554332?text=" + message, "_blank", "noopener");
    });
  }
})();
